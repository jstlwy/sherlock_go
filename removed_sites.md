...
This is a list of sites on which the original Sherlock, as of version 0.14.0,
was unable to determine whether a given username exists. The responses received
from the web sites were listed here so that the original team could devise
better types of requests to send to them so that usernames can be detected.

(The rest of the list is presented in its original, unedited format.)
